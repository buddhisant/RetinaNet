...
def compute_base_anchors():
    ...
    for stride in cfg.fpn_strides:
        ratios=torch.tensor(cfg.anchor_ratio,dtype=torch.float)
        scales=torch.tensor(cfg.anchor_scale,dtype=torch.float)
        w_ratios=torch.sqrt(ratios)
        h_ratios=1/w_ratios

        ws=stride*cfg.anchor_size*w_ratios[:,None]*scales[None,:]
        hs=stride*cfg.anchor_size*h_ratios[:,None]*scales[None,:]
        ws=ws.view(-1)
        hs=hs.view(-1)

        base_anchors_per_level=torch.stack([-0.5*ws, -0.5*hs, 0.5*ws, 0.5*hs],dim=-1)
        base_anchors.append(base_anchors_per_level)
